...
(function(angular, undefined) {
	...
	var DEFAULT_MENU = [
			[ 'bold', 'italic', 'underline', 'strikethrough', 'subscript',
					'superscript' ], [ 'format-block' ], [ 'font' ],
			[ 'font-size' ], ['back-color', 'font-color', 'hilite-color' ],
			[ 'remove-format' ],
			[ 'ordered-list', 'unordered-list', 'outdent', 'indent' ],
			[ 'left-justify', 'center-justify', 'right-justify' ],
			[ 'code', 'quote', 'paragraph' ], [ 'link', 'image' ] ];

	angular
			.module('texteditor.module', [ 'colorpicker.module' ])
			.directive(
					'texteditor',
					function($timeout, texteditorGui, $compile) {
						return {
                            restrict : 'EA',
							template : '<div>'
									+ '<style>'
									+ '   .texteditor-textarea[contentEditable="false"] { background-color:#eee}'
									+ '   .texteditor-select { height:30px;margin-bottom:1px;}'
									+ '   .texteditor-colorpicker { font-family: arial, sans-serif !important;font-size:16px !important; padding:2px 10px !important;}'
									+ '</style>'
									+ '<div class="texteditor-menu" ng-show="editor"></div>'
									+ '<div id="{{textareaId}}" ng-attr-style="margin-bottom:3%;border:2px;height:{{textareaHeight || \'auto\'}}; overflow-y:scroll;mind" contentEditable="{{!disabled}}" class="{{textareaClass}} texteditor-textarea" rows="{{textareaRows}}" ng-maxlength="{{textareaNgMaxlength}}" name="{{textareaName}}" required="{{textareaRequired}}" placeholder="{{textareaPlaceholder}}" ng-model="value"  ng-click="showEditor()" ng-dblclick="hideEditor()"></div>'
									+ '</div>',
									
									/*resize:vertical; -> add this code if textarea want to make resizable*/
							...
							scope : {
                                ...
                                textareaNgMaxlength : '@textareaNgMaxlength',
						...
						function link(scope, element, attrs, ngModelController) {
							...
							var textarea = element.find('div.texteditor-textarea');
							...
							scope.fonts = [ 'Georgia', 'Palatino Linotype',
									'Times New Roman', 'Arial', 'Helvetica',
									'Arial Black', 'Comic Sans MS', 'Impact',
									'Lucida Sans Unicode', 'Tahoma',
									'Trebuchet MS', 'Verdana', 'Courier New',
									'Lucida Console', 'Helvetica Neue' ].sort();
							...
							function compileMenu() {
								texteditorGui
										.setCustomElements(scope.textareaCustomMenu);
								var menuDiv = element
										.children('div.texteditor-menu')[0];
								menuDiv.appendChild(texteditorGui
										.createMenu(scope.textareaMenu));
								$compile(menuDiv)(scope);
							}

							function configureDisabledWatch() {
								scope.$watch('disabled', function(newValue) {
									angular.element('div.texteditor-menu').find(
											'button').each(
											function() {
							...
							function configureListeners() {
								...
								// Send message to calling controller that a
								// button has been clicked.
								angular
										.element('.texteditor-menu')
										.find('button')
										.on(
												'click',
												function() {
								...
								textarea.on('input keyup paste mouseup',
										function() {
								...
								textarea.on('keydown', function(event) {
									if (event.keyCode == 9) {
										...
										// textarea.html(html);
										// selection.collapse(textarea[0].firstChild,
										// position + TAB_SPACES);
									}
								});
								...
								textarea
										.on(
												'click keyup focus mouseup',
												function() {
													$timeout(
															function() {
																scope.isBold = scope
																		.cmdState('bold');
																scope.isUnderlined = scope
																		.cmdState('underline');
																scope.isStrikethrough = scope
																		.cmdState('strikethrough');
																scope.isItalic = scope
																		.cmdState('italic');
																scope.isSuperscript = itemIs('SUP'); // scope.cmdState('superscript');
																scope.isSubscript = itemIs('SUB'); // scope.cmdState('subscript');
																scope.isRightJustified = scope
																		.cmdState('justifyright');
																scope.isLeftJustified = scope
																		.cmdState('justifyleft');
																scope.isCenterJustified = scope
																		.cmdState('justifycenter');
																scope.isPre = scope
																		.cmdValue('formatblock') === 'pre';
																scope.isBlockquote = scope
																		.cmdValue('formatblock') === 'blockquote';

																scope.isOrderedList = scope
																		.cmdState('insertorderedlist');
																scope.isUnorderedList = scope
																		.cmdState('insertunorderedlist');
																...
																element
																		.find(
																				'button.texteditor-hiliteColor')
																		.css(
																				'background-color',
																				scope.hiliteColor);

																scope.fontColor = scope
																	.cmdValue('forecolor');
																element
																		.find(
																					'button.texteditor-fontcolor')
																		.css(
																				'color',
																				scope.fontColor);
							...
							// Used to detect things like A tags and others that
							// dont work with cmdValue().
							function itemIs(tag) {
								var selection = window.getSelection()
										.getRangeAt(0);
								if (selection) {
									if (selection.startContainer.parentNode.tagName === tag
											.toUpperCase()
											|| selection.endContainer.parentNode.tagName === tag
													.toUpperCase()) {
										return true;
									} else {
							...
							scope.setFormatBlock = function() {
								scope.format('formatBlock',
										scope.formatBlock.value);
							};
							...
							scope.setBackColor = function() {
								var id = scope.textareaId;
								/*console.log(id);
								console.log($('#'+id).parents().eq(4));*/
								scope.format('backcolor', scope.backColor);
								$('#'+id).closest('.backGroundColor').css({"background-color":scope.backColor})
			...
			.factory(
					'texteditorGui',
					function(texteditorGuiElements) {

						var ELEMENTS = texteditorGuiElements;
						var custom = {};
						...
						var getMenuGroup = function() {
							return {
								tag : 'div',
								classes : 'btn-group btn-group-sm texteditor-btn-group-margin',
							};
						};
			...
			.value(
					'texteditorGuiElements',
					{
						...
						'strikethrough' : {
							 ...
							 * 'subscript': { tag: 'button', classes: 'btn
							 * btn-default', attributes: [{ name: 'title',
							 * value: 'Subscript' }, { name: 'ng-click', value:
							 * 'format(\'subscript\')' }, { name: 'ng-class',
							 * value: '{ active: isSubscript }' }, { name:
							 * 'type', value: 'button' }], data: [{ tag: 'i',
							 * classes: 'fa fa-subscript' }] }, 'superscript': {
							 * tag: 'button', classes: 'btn btn-default',
							 * attributes: [{ name: 'title', value:
							 * 'Superscript' }, { name: 'ng-click', value:
							 * 'format(\'superscript\')' }, { name: 'ng-class',
							 * value: '{ active: isSuperscript }' }, { name:
							 * 'type', value: 'button' }], data: [{ tag: 'i',
						...
						/*'btn-color' : {
							tag : 'button',
							classes : 'btn btn-default texteditor-colorpicker texteditor-btncolor',
							text : 'BBG',
							attributes : [ {
								name : 'title',
								value : 'Button Color'
						...
						'back-color' : {
							tag : 'button',
							classes : 'btn btn-default texteditor-colorpicker texteditor-backcolor',
							text : 'BG',
							attributes : [ {
								name : 'title',
						...
						'font-color' : {
							tag : 'button',
							classes : 'btn btn-default texteditor-colorpicker texteditor-fontcolor',
							text : 'A',
							attributes : [ {
								name : 'title',
								value : 'Font Color'
						...
						'font' : {
							tag : 'select',
							classes : 'form-control1 texteditor-select',
							attributes : [ {
								name : 'title',
								value : 'Image'
							}, {
